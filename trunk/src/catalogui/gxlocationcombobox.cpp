...
wxGxLocationComboBox::wxGxLocationComboBox(wxWindow* parent, wxWindowID id, const wxSize& size) : wxComboCtrl(parent, id, wxEmptyString, wxDefaultPosition, size), wxGxPathsListViewPopupParent()
{
    m_pApp = NULL;
    m_pSelection = NULL;
    m_pCatalog = NULL;
...
void wxGxLocationComboBox::OnTextEnter(wxCommandEvent& event)
{
	if(m_pCatalog)
	{
        ...
        if(sVal[sVal.Len() - 1] == wxFileName::GetPathSeparator())
