...
#  include <wxgis/core/process/detail/posix_ops.hpp>
#  include <stdlib.h>
#  include <unistd.h>
...
#  include <wxgis/core/process/detail/win32_ops.hpp>
...
inline child launch(const Executable &exe, const Arguments &args, const Context &ctx)
{
    ...
    if (!::CloseHandle(pi.hThread))
        boost::throw_exception(std::runtime_error("boost::process::launch: CloseHandle failed"));
...
inline child launch_shell(const std::string &command, const Context &ctx)
{
    ...
    if (!size)
        boost::throw_exception(std::runtime_error("boost::process::launch_shell: GetWindowsDirectory failed"));
    BOOST_ASSERT(size < MAX_PATH);
...
inline children launch_pipeline(const Entries &entries)
{
...

    {
        ...
        if (!::CloseHandle(pi.hThread))
            boost::throw_exception(std::runtime_error("boost::process::launch_pipeline: CloseHandle failed"));
