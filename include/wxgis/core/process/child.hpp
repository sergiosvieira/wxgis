...
public:
    ...
    status wait()
    {
        ...
        if (::waitpid(get_id(), &s, 0) == -1)
            boost::throw_exception(std::runtime_error("boost::process::child::wait: waitpid(2) failed"));
        ...
        if (!::GetExitCodeProcess(process_handle_.get(), &code))
            boost::throw_exception(std::runtime_error("boost::process::child::wait: GetExitCodeProcess failed"));
        return status(code);
#endif
    }
