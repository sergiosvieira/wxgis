...
public:
    ...
    void posix_remap(handle_type h)
    {
        ...
        if (::dup2(handle_, h) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::file_handle::posix_remap: dup2(2) failed"));
        ...
        {
            ::close(h);
            boost::throw_exception(std::runtime_error("boost::process::detail::file_handle::posix_remap: close(2) failed"));
    ...
    {
        if (::dup2(h1, h2) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::file_handle::posix_dup: dup2(2) failed"));
    ...
    static file_handle win32_dup(HANDLE h, bool inheritable)
    {
        ...
        if (!::DuplicateHandle(::GetCurrentProcess(), h, ::GetCurrentProcess(), &h2, 0, inheritable ? TRUE : FALSE, DUPLICATE_SAME_ACCESS))
            boost::throw_exception(std::runtime_error("boost::process::detail::file_handle::win32_dup: DuplicateHandle failed"));
    ...
    static file_handle win32_std(DWORD d, bool inheritable)
    {
        ...
        if (h == INVALID_HANDLE_VALUE)
            boost::throw_exception(std::runtime_error("boost::process::detail::file_handle::win32_std: GetStdHandle failed"));
    ...
    void win32_set_inheritable(bool i)
    {
        ...
        if (!::SetHandleInformation(handle_, HANDLE_FLAG_INHERIT, i ? HANDLE_FLAG_INHERIT : 0))
            boost::throw_exception(std::runtime_error("boost::process::detail::file_handle::win32_set_inheritable: SetHandleInformation failed"));
    }
#endif
