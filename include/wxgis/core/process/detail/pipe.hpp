...
#if defined(BOOST_POSIX_API)
        if (::pipe(hs) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: pipe(2) failed"));
...
#  if defined(BOOST_PROCESS_WINDOWS_USE_NAMED_PIPE)
        ...
        if (hs[0] == INVALID_HANDLE_VALUE)
            boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: CreateNamedPipe failed"));
        hs[1] = ::CreateFileA(pipe.c_str(), GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
        if (hs[1] == INVALID_HANDLE_VALUE)
            boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: CreateFile failed"));

        OVERLAPPED overlapped;
        ZeroMemory(&overlapped, sizeof(overlapped));
        overlapped.hEvent = ::CreateEvent(NULL, TRUE, FALSE, NULL);
        if (!overlapped.hEvent)
            boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: CreateEvent failed"));
        ...
        if (!b)
        {
            if (::GetLastError() == ERROR_IO_PENDING)
            {
                if (::WaitForSingleObject(overlapped.hEvent, INFINITE) == WAIT_FAILED)
                {
                    ...
                    boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: WaitForSingleObject failed"));
                }
            }
            else if (::GetLastError() != ERROR_PIPE_CONNECTED)
            {
                ::CloseHandle(overlapped.hEvent);
                boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: ConnectNamedPipe failed"));
            }
        }
        ::CloseHandle(overlapped.hEvent);
#  else
        if (!::CreatePipe(&hs[0], &hs[1], &sa, 0))
            boost::throw_exception(std::runtime_error("boost::process::detail::pipe::pipe: CreatePipe failed"));
#  endif
#endif
