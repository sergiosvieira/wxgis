...
#include "wxgis/core/process/detail/file_handle.hpp"
#include "wxgis/core/process/detail/pipe.hpp"
#include "wxgis/core/process/detail/stream_info.hpp"
...
struct posix_setup
{
    ...
    void operator()() const
    {
        if (!chroot.empty() && ::chroot(chroot.c_str()) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::posix_setup: chroot(2) failed"));
        ...
        if (gid != ::getgid() && ::setgid(gid) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::posix_setup: setgid(2) failed"));
        ...
        if (egid != ::getegid() && ::setegid(egid) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::posix_setup: setegid(2) failed"));
        ...
        if (uid != ::getuid() && ::setuid(uid) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::posix_setup: setuid(2) failed"));
        ...
        if (euid != ::geteuid() && ::seteuid(euid) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::posix_setup: seteuid(2) failed"));
        ...
        if (::chdir(work_directory.c_str()) == -1)
            boost::throw_exception(std::runtime_error("boost::process::detail::posix_setup: chdir(2) failed"));
        ...
        case stream_info::use_file:
            {
                ...
                if (fd == -1)
                    boost::throw_exception(std::runtime_error("boost::process::detail::setup_input: open(2) of " + si.file_ + " failed"));
...
inline pid_t posix_start(const Executable &exe, const Arguments &args, const environment &env, info_map &infoin, info_map &infoout, const posix_setup &setup)
{
    pid_t pid = ::fork();
    if (pid == -1)
        boost::throw_exception(std::runtime_error("boost::process::detail::posix_start: fork(2) failed"));
    ...
    else if (pid == 0)
    {
        ...
        std::runtime_error e("boost::process::detail::posix_start: execve(2) failed");
