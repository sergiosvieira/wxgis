...
public:
    ...
    void terminate(bool force = false) const
    {
        ...
        if (::kill(id_, force ? SIGKILL : SIGTERM) == -1)
            boost::throw_exception(std::runtime_error("boost::process::process::terminate: kill(2) failed"));
        ...
        if (h == NULL)
            boost::throw_exception(std::runtime_error("boost::process::process::terminate: OpenProcess failed"));
        if (!::TerminateProcess(h, EXIT_FAILURE))
        {
            ::CloseHandle(h);
            boost::throw_exception(std::runtime_error("boost::process::process::terminate: TerminateProcess failed"));
        }
        if (!::CloseHandle(h))
            boost::throw_exception(std::runtime_error("boost::process::process::terminate: CloseHandle failed"));
#endif
    }
